...
import { Button, Form, Input, Result, App, Progress } from 'antd';
...
const ResetPassword: React.FC = () => {
  ...
  if (!token) {
    return (
      <div style={{
        ...
        <Helmet><title>Liên kết không hợp lệ - {Settings.title}</title></Helmet>
        <Result
          status="error"
          title="Liên kết không hợp lệ"
          subTitle="Liên kết đặt lại mật khẩu không tồn tại hoặc đã hết hạn. Vui lòng yêu cầu liên kết mới."
          extra={<Button type="primary" style={{ background: '#4F46E5', borderColor: '#4F46E5' }}><Link to="/user/forgot-password">Yêu cầu liên kết mới</Link></Button>}
          style={{ background: '#fff', padding: 40, borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
        />
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div style={{
        ...
        <Helmet><title>Đặt lại mật khẩu thành công - {Settings.title}</title></Helmet>
        <Result
          status="success"
          title="Đặt lại mật khẩu thành công!"
          subTitle="Mật khẩu của bạn đã được cập nhật. Bây giờ bạn có thể đăng nhập bằng mật khẩu mới."
          extra={<Button type="primary" size="large" style={{ background: '#4F46E5', borderColor: '#4F46E5' }}><Link to="/user/login">Đăng nhập ngay</Link></Button>}
          style={{ background: '#fff', padding: 40, borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
        />
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <Helmet><title>Đặt lại mật khẩu - {Settings.title}</title></Helmet>
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '32px 0' }}>
        <div style={{
          background: '#fff', borderRadius: 12, padding: '40px 36px',
          width: '100%', maxWidth: 420, boxShadow: '0 4px 16px rgba(0,0,0,0.1)',
        }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <img alt="logo" src="/logo.svg" style={{ width: 44, height: 44, marginBottom: 12 }} />
            <h2 style={{ fontSize: 22, fontWeight: 'bold', color: '#111827', margin: 0 }}>Đặt lại mật khẩu</h2>
          ...
          <Form form={form} layout="vertical" onFinish={handleSubmit}>
            ...
            <Form.Item>
              <Button
                ...
                size="large"
                loading={loading}
                style={{ background: '#4F46E5', borderColor: '#4F46E5', height: 44, fontWeight: 500 }}
              >
                Đặt lại mật khẩu
              </Button>
            </Form.Item>
          ...
          <div style={{ textAlign: 'center' }}>
            <Link to="/user/login" style={{ color: '#4F46E5', fontWeight: 500 }}>
              ← Quay lại đăng nhập
            </Link>
          </div>
        </div>
